# debuilder
This repository periodically (currently once per month) creates a Debian Stretch minimal rootfs from using **debootstrap** for every architecture that Debian supports (I'm currently using these for multi-arch builds on Azure Pipelines, but they should be useful for most scenarios requiring root filesystems for multiple architectures).
